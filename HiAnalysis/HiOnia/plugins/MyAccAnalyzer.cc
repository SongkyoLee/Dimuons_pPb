...
class MyAccAnalyzer : public edm::EDAnalyzer {
public:
  explicit MyAccAnalyzer(const edm::ParameterSet&);
  ~MyAccAnalyzer();
...
private:
  ...
  bool checkCuts(const pat::CompositeCandidate* cand, const pat::Muon* muon1,  const pat::Muon* muon2, bool(MyAccAnalyzer::* callFunc1)(const pat::Muon*), bool(MyAccAnalyzer::* callFunc2)(const pat::Muon*)); 
  ...
  int Reco_QQ_sign[Max_QQ_size];   /* Mu Mu combinations sign:
			     ...
			     2 = -/- 
			  */
  int Reco_QQ_trkSize[Max_QQ_size];      // KYO test
...
//
MyAccAnalyzer::MyAccAnalyzer(const edm::ParameterSet& iConfig):
...
void
MyAccAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  //   using namespace edm;
  InitEvent();
...
void
MyAccAnalyzer::fillRecoHistos(int lastSign) {
...
void
MyAccAnalyzer::fillTreeMuon(const pat::Muon* muon, int iType, int trigBits) {
...
void
MyAccAnalyzer::fillTreeJpsi(int iSign, int count) {
...
*/
  ...
  Reco_QQ_trig[Reco_QQ_size] = trigBits;
	Reco_QQ_trkSize[Reco_QQ_size] = aJpsiCand->userInt("trkSize"); // KYO test
  ...
  if (_muonLessPrimaryVertex && aJpsiCand->hasUserData("muonlessPV")) {
    ...
    cout << "MyAccAnalyzer::fillTreeJpsi: no PVfor muon pair stored" << endl;
...
void
MyAccAnalyzer::fillRecoJpsi(int iSign, int count, std::string trigName, std::string centName) {
...
void
MyAccAnalyzer::fillHistosAndDS(unsigned int theCat, const pat::CompositeCandidate* aJpsiCand) {

  return;
}

void
MyAccAnalyzer::checkTriggers(const pat::CompositeCandidate* aJpsiCand) {
  const pat::Muon* muon1 = dynamic_cast<const pat::Muon*>(aJpsiCand->daughter("muon1"));
  const pat::Muon* muon2 = dynamic_cast<const pat::Muon*>(aJpsiCand->daughter("muon2"));
...
void
MyAccAnalyzer::makeCuts(int sign) {
  ...
  if (collJpsi.isValid()) {
	...
	std::cout << "MyAccAnalyzer::makeCuts: no PV for muon pair stored" << std::endl;
	...
	/*
	// global + global?
	if (checkCuts(cand,muon1,muon2,&MyAccAnalyzer::selGlobalMuon,&MyAccAnalyzer::selGlobalMuon)){
	  _thePassedCats[sign].push_back(GlbGlb);  _thePassedCands[sign].push_back(cand);
	  //	  continue;
	}
		
	// global + tracker? (x2)    
	if (checkCuts(cand,muon1,muon2,&MyAccAnalyzer::selGlobalMuon,&MyAccAnalyzer::selTrackerMuon)){
	  _thePassedCats[sign].push_back(GlbTrk);  _thePassedCands[sign].push_back(cand);
	  //	  continue;
	}

	if (checkCuts(cand,muon2,muon1,&MyAccAnalyzer::selGlobalMuon,&MyAccAnalyzer::selTrackerMuon)){
	  _thePassedCats[sign].push_back(GlbTrk);  _thePassedCands[sign].push_back(cand);
	  //	  continue;
	}
	*/
	...
	// tracker + tracker?  
	if (checkCuts(cand,muon1,muon2,&MyAccAnalyzer::selTrackerMuon,&MyAccAnalyzer::selTrackerMuon)){
...
bool
MyAccAnalyzer::checkCuts(const pat::CompositeCandidate* cand, const pat::Muon* muon1,  const pat::Muon* muon2, bool(MyAccAnalyzer::* callFunc1)(const pat::Muon*), bool(MyAccAnalyzer::* callFunc2)(const pat::Muon*)) {
...
MyAccAnalyzer::theBestQQ(int sign) {
...
bool
MyAccAnalyzer::isMuonInAccept(const pat::Muon* aMuon) {
...
bool
MyAccAnalyzer::selGlobalMuon(const pat::Muon* aMuon) {
...
bool 
MyAccAnalyzer::selTrackerMuon(const pat::Muon* aMuon) {
...
void
MyAccAnalyzer::fillGenInfo() // KYO - diff. with HiOnia (gamma radiation included)
{
...
void
MyAccAnalyzer::fillRecoMuons(int iCent)
{
...
MyAccAnalyzer::InitTree()
{
  ...
  myTree->Branch("Reco_QQ_trkSize", Reco_QQ_trkSize,   "Reco_QQ_trkSize[Reco_QQ_size]/I"); // KYO test
...
void 
MyAccAnalyzer::beginRun(const edm::Run& iRun, const edm::EventSetup& iSetup) {
/*
...
TLorentzVector
MyAccAnalyzer::lorentzMomentum(const reco::Candidate::LorentzVector& p) {
  TLorentzVector res;
...
void
MyAccAnalyzer::hltReport(const edm::Event &iEvent ,const edm::EventSetup& iSetup)
{
  ...
  if ( hltConfigInit ) {
    ...
    for (std::map<std::string, unsigned int>::iterator it = mapTriggernameToHLTbit.begin(); it != mapTriggernameToHLTbit.end(); it++) {
      ...
      if (triggerIndex >= n) {
	//	std::cout << "[MyAccAnalyzer::hltReport] --- TriggerName " << it->first << " not available in config!" << std::endl;
      }
      else {
	it->second= triggerIndex;
	//	std::cout << "[MyAccAnalyzer::hltReport] --- TriggerName " << it->first << " available in config!" << std::endl;
      }
    }
  }
  ...
  try {
    ...
    //    std::cout << "[MyAccAnalyzer::hltReport] --- J/psi TriggerResult is present in current event" << std::endl;
  }
  catch(...) {
    //    std::cout << "[MyAccAnalyzer::hltReport] --- J/psi TriggerResults NOT present in current event" << std::endl;
  }
  if ( collTriggerResults.isValid() ){
    //    std::cout << "[MyAccAnalyzer::hltReport] --- J/psi TriggerResults IS valid in current event" << std::endl;
  ...
  } else std::cout << "[MyAccAnalyzer::hltReport] --- TriggerResults NOT valid in current event" << std::endl;

  return;
}

//define this as a plug-in
DEFINE_FWK_MODULE(MyAccAnalyzer);
